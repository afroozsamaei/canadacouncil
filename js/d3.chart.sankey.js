	...
	module.exports = Base.extend("Sankey", {
		...
		initialize: function() {
				...
				insert: function () {
				        return this.append("path")
                                   ...
                                   .style("display", function (d) {
				                    if (d.source.id == "Total" || d.source.id =="Individual" || d.source.id =="Organisation") {//Do not show the links between third and fourth column
				                            return 'block'
                                    }else {
				                            return 'none'
				                        }
			...
			chart.layer("nodes", chart.layers.nodes, {
				...
				insert: function() {
					return this.append("g").classed("node", true).attr("id", function(d) {
						return d.id;
					})
                    .style("display", function (d) {
				          if (d.column === "4" ) {//Do not show the fourth column 
				                return 'none'
                            }else {
				                return 'block'
				            }
				...
				events: {
					...
					"merge": function() {
						...
						this.select("text")
							...
							.attr("x", function(d) { return (6 + chart.features.nodeWidth);}) //Changed the position of the last column's labels
		...
		_spreadNodes: function(data) {
			...
			nodesByBreadth.forEach(function(nodes) {
				var i,
						...
						padding = (chart.features.height-30 - sum) / nodes.length,
                         y0=0;

				nodes.sort(function(a, b) { return a.y - b.y; });
                
                //Changing the vertical positions of the nodes
				for (i = 0; i < nodes.length; ++i) {
					
                      node = nodes[i];
                     if (node.id =='Total'){
                         node.y = (chart.features.height-sum)/2 - 30;
                     } else if(node.id =='Individual'){
                         node.y = (chart.features.height-sum)/2 - padding+40;
                     } else if (node.id=='Organisation'){
                         node.y = (chart.features.height-sum)/2 + padding-40;
                     } else{
                      node.y = y0;
                      y0 += node.dy + padding;
...
/***/ function(module, exports, __webpack_require__) {
	...
	d3.sankey = function() {
	  ...
	  function scaleNodeBreadths(kx) {
	    nodes.forEach(function(node) {
      //Changing the spacings, so there is more space between the columns with higher number of link exchange - Like this: |  |   |     |
          if (node.x == 1 || node.x==2){
	           node.x *= (0.75*kx);
			  }else if(node.x==3){
				node.x *= (0.9*kx);
		  	}else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = Sankey.extend("Sankey.Selection", {
		...
		initialize: function() {
			...
			chart.features.unselectedOpacity = 1;
			...
			function update() {
				...
				if (chart.features.selection && chart.features.selection.length && !chart.features.selection[0].source) {
